...
func TestCachePolicyEvaluator_EvaluateDecision_Preserve(t *testing.T) {
	...
	shouldFlush, err := evaluator.EvaluateDecision("server1", 1000, 0.9, false, "workflow1", "")
	require.NoError(t, err)
	assert.False(t, shouldFlush)

	// Even with final iteration, preserve should still preserve (unless flush_after_final is set)
	shouldFlush, err = evaluator.EvaluateDecision("server1", 1000, 0.9, true, "workflow1", "")
	require.NoError(t, err)
	assert.False(t, shouldFlush)
}
...
func TestCachePolicyEvaluator_EvaluateDecision_AggressiveFlush(t *testing.T) {
	...
	shouldFlush, err := evaluator.EvaluateDecision("server1", 10, 0.1, false, "workflow1", "")
	require.NoError(t, err)
	assert.True(t, shouldFlush)
...
func TestCachePolicyEvaluator_EvaluateDecision_PreserveOnSmallTurns(t *testing.T) {
	...
	shouldFlush, err := evaluator.EvaluateDecision("server1", 50, 0.5, false, "workflow1", "")
	require.NoError(t, err)
	assert.False(t, shouldFlush)

	// Large turn should flush
	shouldFlush, err = evaluator.EvaluateDecision("server1", 150, 0.5, false, "workflow1", "")
	require.NoError(t, err)
	assert.True(t, shouldFlush)
...
func TestCachePolicyEvaluator_EvaluateDecision_FlushUnderPressure(t *testing.T) {
	...
	shouldFlush, err := evaluator.EvaluateDecision("server1", 1000, 0.5, false, "workflow1", "")
	require.NoError(t, err)
	assert.False(t, shouldFlush)
	...
	shouldFlush, err = evaluator.EvaluateDecision("server1", 1000, 0.8, false, "workflow1", "")
	require.NoError(t, err)
	assert.False(t, shouldFlush)
}
